...
class NewNeuralNet():
    ...
    def predict(self, X,y_test,X_train,y_train):
        ytilde = self.feed_forward_out(X)
        ytilde = ytilde.reshape(-1) #ravel()

        print("Values calculated for test data:")
        print("The NeuralNetwork MSE score is: ",functions.MSE(y_test,ytilde))
        print("The NeuralNetwork R2 score is: ",functions.R2(y_test,ytilde))       
        
        print("\nValues calculated for training data:")
        ytilde = self.feed_forward_out(X_train)
        ytilde = ytilde.reshape(-1)         
        print("The NeuralNetwork MSE score is: ",functions.MSE(y_train,ytilde))
        print("The NeuralNetwork R2 score is: ",functions.R2(y_train,ytilde))            
    ...
    def train(self):
        ...
        print("\nChosen activation function is: ",self.activation_f)         
